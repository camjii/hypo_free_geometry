...
null_cloud.py has since evolved (different Manifold signature).
"""
...
class ManifoldComparator:
    ...
    def diagram_distance(self, m1, m2, max_dim=1):
        ...
        for k in range(min(max_dim + 1, len(m1.dgms), len(m2.dgms))):
            ...
            # persim chokes on empty diagrams (common for H1 at small n).
            # Both empty: identical, distance 0. One empty: every bar in the
            # other matches to the diagonal at cost (death - birth) / 2.
            if not len(a) and not len(b):
                out[f"H{k}"] = {"wasserstein": 0.0, "bottleneck": 0.0}
                continue
            if not len(a) or not len(b):
                half_pers = (lambda d: (d[:, 1] - d[:, 0]) / 2)(a if len(a) else b)
                out[f"H{k}"] = {"wasserstein": float(half_pers.sum()),
                                "bottleneck": float(half_pers.max())}
                continue
            out[f"H{k}"] = {
